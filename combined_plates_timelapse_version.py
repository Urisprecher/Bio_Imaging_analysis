        ...
        for subfolder_name in os.listdir(main_folder):
            ...
            #editing column names
                    try:
                        ...
                        logging.info("Column names cleaned, see in csv files the new names.")
                    except Exception as e:
...
def process_data_combine(folder_path):
    try:
        ...
        output_folder = os.path.join(main_folder, "Timelapse_result_combined")
        os.makedirs(output_folder, exist_ok=True)
        result_combine_dir = os.path.join(main_folder, "Timelapse_result_combined/combined_csv_file")
        ...
        combined_df['well'] = combined_df['Row'].astype('str') + combined_df['Column'].astype('str') + combined_df['Timepoint'].astype('str')
...
def process_plate_data_on_folders(main_folder, unwanted_columns, important_features, index_columns):
    try:
        ...
        index_columns = [str(element) for element in index_columns]
            ...
            if os.path.isdir(item_path):
                ...
                for file_name in os.listdir(item_path):
                    if file_name.endswith(".csv"):
                        ...
                        try:
                            missing_features = [feature for feature in important_features if
                                                feature not in plate_df.columns]
                            if missing_features:
                            ...
                            logging.error(f"There was an issue with important features...{e}")
                        try:
                            if 'well' in index_columns:
                                index_columns.remove("well")
                        except Exception as e:
                            logging.error("well column does not exists yet, creating now...")
                        #create well column and update index_columns
                        try:
                            print(index_columns)
                            plate_df['well'] = plate_df['Row'].astype(str) + plate_df['Column'].astype(str)+ plate_df['Timepoint'].astype('str')
                            plate_df['well'] = plate_df['well'].astype(str)
                            index_columns.append('well')
                            logging.info("Created 'well' column and updated index_columns.")
                        except Exception as e:
                            logging.error(f"Error creating 'well' column: {e}")
                        ...
                        try:
                            ...
                            for cols in new_index_columns:
                                ...
                                if col_name not in plate_df.columns:
                                    new_cols[col_name] = plate_df[list(cols)].apply(lambda x: '_'.join(map(str, x)),
                                                                                    axis=1)
                                else:
                        ...
                        ##add cell count feat
                        try:
                            print("usual cell_count_feat names: All Cells - Number of Objects")
                            cell_count_feat = input("Enter appropriate based on your data, cell_count_feat: ")
                        ...
                        try:
                            print("Section to remove rows based on chosen column, choose column following by values to remove, otherwise, type no")
                            while True:
                                print("Columns in the dataset:")
                                for idx, column in enumerate(plate_df_2.columns):
                                    print(f"{idx}: {column}")
                                column_to_filter = input("Enter the column name to filter rows by (or 'no' to skip): ").strip()
                                if column_to_filter.lower() == 'no':
                                    break
                                if column_to_filter not in plate_df_2.columns:
                                    print("invalid column name. Please try again...")
                                    continue
                                print(f"These are the values in the selected column '{column_to_filter}':")
                        ...
                        try:
                            ...
                            remove_id = input("Enter cell_id values to remove (or 'none'): ").strip()
                            if remove_id.lower() != 'none':
                                plate_df_2 = plate_df_2[~plate_df_2['cell_id'].astype(str).str.contains(remove_id)]
                                logging.info(f"Rows with cell_id '{remove_id}' removed.")
                        except KeyError:
                            logging.warning("Column 'cell_id' not found. skipping cell_id removal.")
                        except Exception as e:
                            logging.error(f"Error removing rows based on cell_id: {e}")
                        #save first df
                        try:
                            plate_df_2.to_csv(os.path.join(results_folder, "df_proc_1.csv"), index=False)
                            logging.info("Saved processed data frame number 1")
                        except Exception as e:
                            logging.error(f"Failed to save initial processed data frame... {e}")
                        ...
                        try:
                            if cell_count_feat in plate_df_2.columns:
                                ...
                                logging.info(f"Histogram saved to {histogram_file_path}")
                            else:
                        ...
                        try:
                            if cell_count_feat in plate_df_2.columns:
                                ...
                                logging.warning("'selected_cells - Number of Objects' column not found. Statistics not generated.")
                        except Exception as e:
                        ...
                        try:
                            ...
                            logging.info("Saved processed data frame number 2")
                        except Exception as e:
                            logging.error(f"Failed to save second processed data frame... {e}")
                        ...
                        try:
                            ...
                            group_columns = ['cell_type', 'compound', 'concentration', 'Timepoint']
                            ...
                            print("excel summary file generated!")
                        ...
                        try:
                            ...
                            group_columns = ['cell_type', 'compound', 'concentration', 'cell_id', 'Timepoint']
                            ...
                            plt.close()
                            print("Number of cells bar plot generated!")
                        except Exception as e:
                        ...
                        #PCA based on all numeric features without cell_id
                        try:
                            print("Creating pcas for group with and without cell_id")
                            ...
                            label_without_id = plate_df_2['cell_type'].astype(str) + '_' + \
                                            ...
                                            plate_df_2['Timepoint'].astype(str)
                            ...
                            fig.update_layout(legend_title='Cell Type, Compound, Concentration, Timepoint')
                            ...
                            #label for each unique combination of cell_type, compound, concentration, and cell_id
                            ...
                            fig = px.scatter(pca_df2, x='PC1', y='PC2', color='Label',
                                             ...
                                             color_continuous_scale='viridis')
                            fig.update_layout(legend_title='Cell Type, Compound, Concentration, ID, Timepoint')
                            fig.add_annotation(text=f'Explained Variance Ratio: {pca.explained_variance_ratio_}', xref='paper',
                                               yref='paper',
                                               x=0.98, y=0.02, showarrow=False, font=dict(size=10))
                        ...
                        try:
                            ...
                            if perform_outlier_detection == 'yes':
                                ...
                                def LOF_outlier(df, output_file1, output_file2):
                                    ...
                                    print(f"running LOF on the following groups...{types}")
                                    mask = []
                                    features = df.select_dtypes(include=np.number).columns
                                    print(f"running LOF on the following features...{features}")
                                    detector_list = [
                            ...
                            logging.error(f"failed to run outlier detection...{e}")
                            out_dat = plate_df_2.copy()
                        #final filtered DataFrame
                        ...
                        try:
                            print(Fore.RED + "==== Please make sure you choose all needed index columns - PC & Timepoint are must! ====")
                            columns_to_set_index = input("Enter ALL needed columns to set as indexes -PC & Timepoint IS MUST! (comma-separated): ").split(',')
                        ...
                        try:
                            print("Starting feature quality control assesment!")
                            #user to select the correct index title as the target variable
                            if out_dat.index.names:
                                logging.info("Select the correct index title to be the target variable for feature QC ( e.g- number corresponding to the correct index:")
                            ...
                            rename_columns = input("Do you want to rename any columns? (yes/no): ").strip().lower()
                            if rename_columns == 'yes':
                                ...
                                renaming_input = input(
                                    "Enter the column name and new name separated by ':', each pair separated by commas (e.g., col1:new_col1,col2:new_col2): ").strip()
                                try:
                            ...
                            logging.error(f"An error occurred during the outlier detection and feature selection process: {e}")
                            return
                        ## time point plots
                        time_plots_dir = os.path.join(results_folder, "time_plots")
                        os.makedirs(time_plots_dir, exist_ok=True)
                        try:
                            print("Time_point plots began!")
                            t_plot_df = out_dat.copy()
                            t_plot_df = t_plot_df.reset_index()
                            # Choose index column based on available ones
                            t_plot_df = t_plot_df.reset_index(drop=True)
                            print("Available columns for time_point plots:", t_plot_df.columns)
                            index_col_main = input("Enter the name of the index column: ")
                            t_plot_df.set_index(index_col_main, inplace=True)
                            timepoint_col = "Timepoint"
                            numeric_cols = t_plot_df.select_dtypes(include=[np.number]).columns
                            # numeric_cols = numeric_cols.drop(timepoint_col)
                            t_plot_df[numeric_cols] = np.log1p(t_plot_df[numeric_cols])
                            #save processed DF
                            processed_file_path = os.path.join(time_plots_dir, 'time_plot_data.csv')
                        ...
                        try:
                            print("Imputing data to handle missing values...")
                            ...
                            logging.info(f"Processed data saved to {data_3_path}")
                        ...
                        print("generating dashboard for your analysis!")
...
def gather_user_lists():
    ...
    print("usual unwanted columns: Number of Analyzed Fields, Height, Plane, Time, Cell Count")
    print("enter unwanted columns:")
    list1 = input("enter unwanted columns:(comma-separated): ").split(',')

    print("usual important columns: spots_chanel_3 - Corrected Spot Intensity - Mean per Well, modified_ir_chanel_3_at - Intensity modified_ir_chanel_3_at chanel_3 Mean - Mean per Well, modified_ir_chanel_4_at - Intensity modified_ir_chanel_4_at chanel_4 Mean - Mean per Well, All Cells - chanel_3 Gabor Max 2 px w2 - Mean per Well, All Cells - chanel_3 SER Hole 1 px - Mean per Well, All Cells - chanel_2 Area, All Cells - chanel_1 Area, All Cells - chanel_4_intensity Mean - Mean per Well, All Cells Selected - chanel_3_intensity Mean - Mean per Well, All Cells - chanel_3_intensity Mean - Mean per Well, All Cells Selected - chanel_4_intensity Mean - Mean per Well, spots_3_per_Cell, mcherry_cy5, All Cells - chanel_2_intensity Mean - Mean per Well, All Cells - chanel_1_intensity Mean - Mean per Well")
    print("enter important_features:")
    list2 = input("enter important_features:(comma-separated): ").split(',')

    print("must index columns!!!: PC, cell_type, cell_id, compound, concentration, Timepoint")
...
def main(main_folder, main_folder_path, final_folder_path):
    print(Fore.GREEN + "Starting Timelapse IF analysis for combined files of all plates in folder in your main directory:)")
    ...
    print("Completed combined Timelapse IF analysis, good luck with statistics:)")
...
if __name__ == "__main__":
    ...
    final_folder_path = input("Enter: the main folder name/Timelapse_result_combined ")
